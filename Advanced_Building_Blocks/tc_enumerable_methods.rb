...
class TestEnumerableMethods < Test::Unit::TestCase
	...
	def test_my_each_with_index
		...
		[13, 15, 17, 19].my_each_with_index {|element, index| \
	...
	def test_my_select
		assert_equal([1, 2], [1, 2, "hello", [], :item].my_select \
	...
	def test_my_all?
		assert_equal(true, [2, 4, 6, 8].my_all? {|item| item%2==0})
		assert_equal(true, %w[ant bear cat].my_all? { |word| \
																								word.length >= 3})
		assert_equal(false, %w[ant bear cat].my_all? { |word| \
																								 word.length >= 4 })
		assert_equal(false, [nil, true, 99].my_all?)
	end

	def test_my_any?
		assert_equal(true, ["A", [], 13, [], true].my_any? {|item|\
																	item.is_a?(String)})
		assert_equal(true, %w[ant bear cat].my_any? {|word| \
																								word.length >= 3})
		assert_equal(true, [nil, true, 99].my_any?)
	end

	def test_my_none?
		assert_equal(false, [15, 20, [], true, "buy"].my_none? \
														{|item| item.is_a?(String)})
		assert_equal(true, %w{ant bear cat}.my_none? { |word| \
										word.length == 5 })
	...
	def test_my_map
		...
		assert_equal([1, 3, 3, 4], [3, 6, 5, 8].my_map {|item| \
														item * 3 / 5 })
		assert_equal([1.8, 3.6, 3.0, 4.8], [3, 6, 5, 8].my_map(&test_proc))
	end

	def test_my_inject
		assert_equal(120, (1..15).to_a.inject {|running_total, item| \
											running_total + item })
	end
